module PF3d
  class Projector
    ...
    def initialize(@width, @height, @camera = Camera.new)
      @clipping_plane_near = PF2d::Vec[0.0, 0.0, @near]
      @clipping_plane_far = PF2d::Vec[0.0, 0.0, @far]
    end
    ...
    def project(tris : Array(Tri), camera = @camera, sort : Bool = false)
      ...
      tris = tris.map do |tri|
        tri.shade = (tri.normal.dot(light) + 1.0) / 2 # light should be normalized
        tri * mat_view
      end

      # Clip tris
      {
        {clipping_plane_near, PF2d::Vec[0.0, 0.0, 1.0]},
        {clipping_plane_far, PF2d::Vec[0.0, 0.0, -1.0]},
      }.each do |clip|
      ...
      {
        {PF2d::Vec[0.0, 0.0, 0.0], PF2d::Vec[0.0, 1.0, 0.0]},
        {PF2d::Vec[0.0, height - 1.0, 0.0], PF2d::Vec[0.0, -1.0, 0.0]},
        {PF2d::Vec[0.0, 0.0, 0.0], PF2d::Vec[1.0, 0.0, 0.0]},
        {PF2d::Vec[width - 1.0, 0.0, 0.0], PF2d::Vec[-1.0, 0.0, 0.0]},
      }.each do |clip|
        0.upto(tris.size - 1) do
          tri = tris.pop
          tri.clip(plane: clip[0], plane_normal: clip[1]).each do |tri|
