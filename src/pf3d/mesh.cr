module PF3d
  ...
  class Mesh
    ...
    def self.load_obj(path, use_normals : Bool = false)
      verticies = [] of PF2d::Vec3(Float64)
      texture_verticies = [] of PF2d::Vec3(Float64)
      normal_verticies = [] of PF2d::Vec3(Float64)
      tris = [] of Tri
      ...
      File.open(path) do |file|
        file.each_line do |line|
          ...
          when "vn"
            # Vertex Normal
            if use_normals
              normal_verticies << PF2d::Vec[parts[1].to_f64, parts[2].to_f64, parts[3].to_f64]
            end
          when "f"
            # Face
            # EX: f 1/1 2/2 3/3 4/4

            face_verts = [] of PF2d::Vec3(Float64)
            face_tex = [] of PF2d::Vec3(Float64)

            normal : PF2d::Vec3(Float64)? = nil
    ...
    def tris
      ...
      @tris.map do |tri|
        tri *= Transform.scale(scale)
        tri *= Transform.translation(origin)
        tri *= Transform.rotation(rotation)
        tri *= Transform.translation(position)
        tri
      end
    end
  end
